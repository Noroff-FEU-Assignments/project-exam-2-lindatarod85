...
const ReactionView = ({ reactionId, reactionsCount, onNewReaction }) => {
  ...
  useEffect(() => {
    fetchReactions();
  }, [reactionsCount, fetchReactions]); // Only fetch reactions when reactionsCount changes

  useEffect(() => {
    // Check if onNewReaction is defined and call fetchReactions if it is
    if (typeof onNewReaction === "function") {
      fetchReactions();
    }
  }, [onNewReaction, fetchReactions]); // Only call fetchReactions when onNewReaction changes
